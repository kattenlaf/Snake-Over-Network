...
const char* clearcommand = "clear";        // Terminal Command (Linux: clear | Windows : clr )
char svr_command;
...
/// Declaring Global Temporary Variable To Save Memory
int bytesWaiting, i;
int px,py,nx, ny;
char k;
int h,w;
int x,y;
int movementx=1;                /// Snake Movement
...
void loop(void){
    ...
    while(x<500){
        ...
        // Need to get key separately here
        // Keys read from client to server should be stored, then pulled to use in the 
        // reaction_on_keyboard command
        // Key input will be read from file, and move the snake accordingly
        FILE *fp;
        int c;
        int n = 0;
        ...
        printf("Command from file is!%c\n", c);
        fclose(fp);

        reaction_on_keyboard(c);
